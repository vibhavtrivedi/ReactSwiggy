...
const BodyComponent = () => {
  ...
  const fetchData = async () => {
    ...
    console.log("resturant card", jsonData?.data?.cards[1].card?.card?.gridElements?.infoWithStyle?.restaurants);
    setListOfResturants(jsonData?.data?.cards[1].card?.card?.gridElements?.infoWithStyle?.restaurants);
    setFilteredResturants(jsonData?.data?.cards[1].card?.card?.gridElements?.infoWithStyle?.restaurants);
  }
        ...
        {filteredResturants.map((data) => {
          return (
            <Link key={data.info.id} to={"/resturant/" + data.info.id} >
              {data.info.sla.deliveryTime > 28 ? <ResturantPromoted resData={data} /> : <ResturantCard resData={data} />}
